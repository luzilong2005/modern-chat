<template>
    <div class="bg-mc-background h-screen w-screen">
        <AppTitleBar
            :minimizable="false"
            :maximizable="false"
            :title="t('pages.dialog.model.edit')"
        />
        <ElForm
            class="px-5"
            ref="form"
        ...
        >
            ...
            <ElFormItem :label="t('pages.dialog.model.form.apiKey')">
                <ElInput
                    v-model:model-value="data.apiKey"
                    show-password
                />
            </ElFormItem>
...
const data = reactive<AIModelConfig>({ ...settings.modelConfigs[id.value] });
...
const handleSubmit = async () => {
    ...
    if (!valid) return;
    settings.modelConfigs[id.value] = data;
    ipc.invoke("dialog:close", "edit-model-dialog", toRaw(data));
};
const handleCancel = () => {
    ipc.invoke("dialog:close", "edit-model-dialog", null);
};
</script>
